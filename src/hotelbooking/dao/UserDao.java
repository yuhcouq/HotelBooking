...
public class UserDao {
	...
	public int editIdHotel(User user,int id) {
		String sql = "UPDATE user SET hotel_id = ? WHERE id_user = ? ";
		return jdbcTemplate.update(sql,
	...
	public int addUser(Login login, String date) {
		String sql = "INSERT INTO user(role_id,hotel_id, email, password, firstname, lastname, gender, CreatedAT, latitude, longitude) VALUES (4,?,?,?,?,?,1,?,?,?)";
		return jdbcTemplate.update(sql,
				new Object[] { -1,login.getEmailSignUp(), login.getPasswordSignUp(), login.getFirstnameSignUp(),
